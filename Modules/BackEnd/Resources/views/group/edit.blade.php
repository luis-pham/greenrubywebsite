...
@section('content')
    ...
    {{ Form::open(['route' => [Utilities::getRouteName('backend.group.update'), ['languageCode' => $languageCode, 'typeName' => $typeName, 'id' => $obj->id, 'lastUrl' => Request::get('lastUrl')]], 'id' => 'frm']) }}
        <div class="row">
            <div class="col-lg-8">    
                <div class="card">
                        ...
                        <div class="form-horizontal">
                            ...
                            @if ($type == config('backend.groupType.expActivity'))
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-8">
                                        {{ Form::select('tab', $listTab, $obj->tab, ['class' => 'form-control', 'placeholder' => 'Chọn', 'autocomplete' => 'off']) }}
                                    </div>
                                </div>
                            @endif
                            <div class="form-group row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <select name="category_key" class="form-control">
                                        ...
                                        <option value="dining" {{ old('category_key', $obj->category_key ?? '') == 'dining' ? 'selected' : '' }}>Dining & Social</option>
                                        <option value="pools" {{ old('category_key', $obj->category_key ?? '') == 'pools' ? 'selected' : '' }}>Pools & Outdoors</option>
                                        <option value="wellness" {{ old('category_key', $obj->category_key ?? '') == 'wellness' ? 'selected' : '' }}>Wellness & Activities</option>
                                        <option value="events" {{ old('category_key', $obj->category_key ?? '') == 'events' ? 'selected' : '' }}>Events</option>
                                    </select>

                ...
                <table class="table table-striped table-data">
                    <thead>
                        <tr>
                            ...
                            <th>Du thuyền</th>
                            <th class="text-center" style="width: 120px;">Năm đóng</th>
                            <th class="text-center" style="width: 120px;">Sức chứa</th>
                            <th class="text-center" style="width: 120px;">Hạng</th>
                    ...
                    <tbody>
                        @for ($i = 0; $i < count($list); $i++)
                            @php
                                $dataObj = SourceDataUtils::bindSourceDataCruiseDetail($list[$i], $languageCode);
                            @endphp
                            <tr data-obj="{{ json_encode($dataObj, JSON_UNESCAPED_UNICODE) }}">
                                        ...
                                        <div class="media">
                                            @if ($list[$i]->image_link)
                                                <div class="image-wrapper image-16-9 position-relative mr-3">
                                                    {{-- <a href="{{ Utilities::getUrlWithGoBack(route(Utilities::getRouteName('backend.cruise.show'), ['languageCode' => $languageCode, 'id' => $list[$i]->id])) }}" target="_blank"> --}}
                                ...
                                <td class="text-center">
                                    <span class="text-warning">
                                        {{ $list[$i]->star_rating }} <i class="fas fa-star"></i>
                                    </span>
                                </td>
                                <td class="text-center">

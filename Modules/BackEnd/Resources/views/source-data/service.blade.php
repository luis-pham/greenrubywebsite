                ...
                <table class="table table-striped table-data">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" style="width: 120px;">Loại hình</th>
                    ...
                    <tbody>
                        @for ($i = 0; $i < count($list); $i++)
                            @php
                                $dataObj = SourceDataUtils::bindSourceDataServiceDetail($list[$i], $languageCode);
                            @endphp
                            <tr data-obj="{{ json_encode($dataObj, JSON_UNESCAPED_UNICODE) }}">
                                        ...
                                        <div class="media">
                                            @if ($list[$i]->image_link)
                                                <div class="image-wrapper image-16-9 position-relative mr-3">
                                                    {{-- <a href="{{ Utilities::getUrlWithGoBack(route(Utilities::getRouteName('backend.service.show'), ['languageCode' => $languageCode, 'id' => $list[$i]->id])) }}" target="_blank"> --}}
                                ...
                                <td class="text-center">
                                    @if ($list[$i]->group_id)
                                        @if (\Auth::user()->can('group-service-read'))
                                            <a href="{{ Utilities::getUrlWithGoBack(route(Utilities::getRouteName('backend.group.show'), ['languageCode' => $languageCode, 'typeName' => 'service', 'id' => $list[$i]->group_id])) }}" target="_blank">
                                                {{ $list[$i]->group_name }}
                                            </a>
                                        @else
                                            {{ $list[$i]->group_name }}
                                ...
                                <td class="text-center">{!! CruiseUtils::formatDisplayServiceType($list[$i]->type) !!}</td>
                                <td class="text-center"><span class="text-info font-weight-bold">{{ Utilities::formatDisplayCurrency($list[$i]->price) }}</span></td>
                                <td class="text-center">
